function main() {
  ...
  let [orders, ordersMissing] = processOrdersData(ordersSheet, incomeSheet);
  ...
  var missingSheet = spreadsheet.getSheetByName('missing');
  if (!missingSheet) {
    missingSheet = spreadsheet.insertSheet('missing');
  } else { 
    missingSheet.clear();
  }
  
  reportMissingOrder(ordersMissing, missingSheet);
...
function reportMissingOrder(missingList, missingSheet) {
  missingList.forEach(function (order) {
    missingSheet.appendRow(order);
  });
}